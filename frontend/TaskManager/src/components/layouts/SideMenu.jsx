...
const SideMenu = ({activeMenu}) => {
  ...
  return <div className='w-64 h-[calc(100vh-61px)] bg-white border-r border-gray-200/50 sticky top-[61px] z-20' >
    <div className='flex flex-col items-center justify-center mb-7 pt-5'>
        <div   className='relative' >
            <img src={user?.profileImageUrl || ""} alt="profile picture" className='w-20 h-20 bg-slate-400 rounded-full ' />

        </div>
        {user?.role ==="admin" &&(
            <div className='text-[10px] font-medium text-white bg-primary py-0.5 px-3 rounded mt-5'>
                    Admin
 ...
 {sideMenuData.map((item,index) => (
    <button className={`w-full flex items-center gap-4 text-[15px] ${
